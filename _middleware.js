export async function onRequest(context) {
    ...
    if (path === '/debug-keys') {
        ...
        return new Response(JSON.stringify({
            raw: { keyIdLength: rawKeyId.length, keyIdPrefix: rawKeyId.substring(0, 15) },
            cleaned: { keyIdLength: cleanKeyId.length, keyIdPrefix: cleanKeyId.substring(0, 12) },
            secret: { keySecretLength: (env.RAZORPAY_KEY_SECRET || '').length }
    ...
    if (path === '/api-test') {
        return new Response(JSON.stringify({
            ...
            endpoints: ['/create-order', '/debug-keys']
        }), { status: 200, headers: corsHeaders });
    }
